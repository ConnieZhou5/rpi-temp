...
const char codeVersion[4] = "0.3";  // code version number
const int timeDefault = 100;  // default duration of data stream, seconds
    ...
    if (bSave == 0) {
        ...
        for (i = 0; i < samples; i++) {
            ...
            if (bytes == 7) {
                ...
                // fabs(x_acc) > 1.5 || fabs(y_acc) > 1.5 || fabs(z_acc) > 1.5
                if (fabs(x_acc) > 1.5 || fabs(y_acc) > 1.5 || fabs(z_acc) > 1.5) {
                    printf("Sending simulated alert to server...\n");
